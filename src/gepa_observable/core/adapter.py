...
# EvaluatorFn now returns feedbacks as a third element (optional)
EvaluatorFn = Callable[[list[DataInst], Candidate], tuple[list[RolloutOutput], list[float], list[str | None] | None]]
...
class EvaluationBatch(Generic[Trajectory, RolloutOutput]):
    ...
    - feedbacks: optional per-example feedback strings from the metric. These are extracted
      when the metric returns a Prediction/dict with a 'feedback' field.
    """
