...
1. Run the examples/eg_v2_simple.py with the server attached
2. We validate if the optimization is running fine, and fix any errors in the example itself
3. We validate that the server is receiving all the logs correctly. This includes:
...
4. Is the lineage being updated correctly? Are we able to compare two prompts in the lineage and see their performance comparison (as detailed as in the iterations tab)?
5. Is the overview being updated correctly? Are we able to see all the data, the prompt comparisons and performance comparisons as detailed as the iterations tab?
